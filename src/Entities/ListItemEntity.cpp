...
void ListItemEntity::setPosition(int positionInt)
{
    if (positionInt < 0) {
...
void ListItemEntity::setListName(std::string listNameStr)
{
    this->listName = std::move(listNameStr);
}

void ListItemEntity::setListVariant(std::string listVariantStr)
{
    this->listVariant = std::move(listVariantStr);
}

void ListItemEntity::setValue(const std::string& valueStr)
{
    if (valueStr.empty()) {
...
void ListItemEntity::setPriority(const PriorityEntity& priorityEntity)
{
    this->priorityEntity = priorityEntity;
}

void ListItemEntity::setStatus(const StatusEntity& statusEntity)
{
    this->statusEntity = statusEntity;
}

void ListItemEntity::setDueAt(time_t dueAtDate)
{
...
void ListItemEntity::setClosedAt(time_t closedAtDate)
{
    // if not timestamp, throw exception
    if (closedAtDate < 0) {
        throw std::invalid_argument("Created at timestamp cannot be negative");
...
void ListItemEntity::setCreatedAt(time_t createdAtDate)
{
    // if not timestamp, throw exception
    if (createdAtDate < 0) {
        throw std::invalid_argument("Created at timestamp cannot be negative");
...
void ListItemEntity::setUpdatedAt(time_t updatedAtDate)
{
    // if not timestamp, throw exception
    if (updatedAtDate < 0) {
        throw std::invalid_argument("Created at timestamp cannot be negative");
...
ListItemEntity ListItemEntity::set(const std::string& id,
...
ListItemEntity ListItemEntity::setFromVector(PriorityService& priorityService, StatusService& statusService, const std::vector<std::string> &item, const std::string &listName, const std::string &listVariant)
{
    ListItemEntity listItemEntity;
