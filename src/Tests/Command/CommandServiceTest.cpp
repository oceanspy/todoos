...
TEST_CASE("CommandService Tests", "[CommandService]")
{
    ...
    SECTION("Get subcommand")
    {
        ...
        REQUIRE(commandService.getSubCommand(command4).getOptions() ==
                std::map<std::string, std::string>{ { "list", "test" } });
        ...
        REQUIRE(commandService.getSubCommand(command5).getOptions() ==
                std::map<std::string, std::string>{ { "list", "test" } });
    }
}
