...
CommandService::CommandService(CommandList& commandList, CommandOption& commandOption)
    : commandList(commandList), commandOption(commandOption)
{
    // Constructor implementation
}

bool CommandService::isValid(std::string& commandName)
{
    return commandList.isValid(commandName);
}


bool CommandService::hasSubCommand(Command command)
{
    return !command.getArguments().empty();
}

Command CommandService::getSubCommand(Command command)
{
...
bool CommandService::isCommand(Command& command, const std::string &commandName)
{
    return CommandShortcut::isCommand(command, commandName);
}

bool CommandService::isBeginningOfCommand(Command command)
{
    return commandList.isBeginningOfCommand(command.getName());
}

bool CommandService::isCommandValidWithOption(std::string commandName, std::map<std::string, std::string> options)
{
    return CommandList::isCommandValidWithOptions(commandName, options);
}
std::vector<std::string> CommandService::getBaseCommandList()
{
    return commandList.getMainCommandNames();
}

std::string CommandService::getMainCommandListAsString()
{
...
CommandList &CommandService::list()
{
    return commandList;
}

CommandOption &CommandService::options()
{
    return commandOption;
}

std::string CommandService::getCommandName(const std::string &shortcut)
{
