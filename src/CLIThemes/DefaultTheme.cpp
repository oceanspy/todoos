...
DefaultTheme::DefaultTheme(IOService& ioService,
                  ...
                  ) : ThemeAbstract(ioService, listService, listItemService, consoleWidth), consoleRowLength(consoleRowMaxLength - 56)
{
}

void DefaultTheme::print(std::string currentListName, std::string currentListVariantName, std::vector <ListItemEntity> listItems, bool showListName, bool showTitle)
{
...
std::string DefaultTheme::buildTitle()
{
    std::string line = "";
    // ID
...
std::string DefaultTheme::buildLine(const ListItemEntity& listItemEntity, bool hideListNameInLine)
{
    std::string line = "";
    // ID
...
void DefaultTheme::printListName()
{
    ...
    std::string pauseCount = std::to_string(listItemService.load().countWithStatus({StatusService::PAUSED, StatusService::PENDING}));
...
void DefaultTheme::printATitle(std::string titleLine1, std::string titleLine2)
{
    int titleLength = titleLine1.length() > titleLine2.length()
                      ? static_cast<int>(titleLine1.length())
                      : static_cast<int>(titleLine2.length());

    if (consoleRowLength > titleLength * 2) {
        consoleRowLength = titleLength * 2 > 60
                ? titleLength * 2
                : 60;
    }
...
void DefaultTheme::renderListStatLine(time_t from, time_t to, std::string name)
{
...
void DefaultTheme::printFullLine(std::string color)
{
    if (color.empty())
    {
        color = GRAY;
...
void DefaultTheme::printAList(std::vector<std::string> lines)
{
...
void DefaultTheme::printAListTitle(std::vector<std::string> titles, std::vector<int> titleSizes)
{
    std::string string;
