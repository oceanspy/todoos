...
ConfigRepository::ConfigRepository(FileDataServiceInterface* fileDataService, const std::filesystem::path filePath)
    : fileDataService(fileDataService), filePath(filePath)
{
...
ConfigEntity ConfigRepository::find(const std::string& key)
{
    fileDataService->load(filePath);
...
void ConfigRepository::create(ConfigEntity item)
{
    fileDataService->load(filePath);
...
bool ConfigRepository::update(const std::string& key, const ConfigEntity& item)
{
    fileDataService->load(filePath);
...
bool ConfigRepository::remove(std::string& key)
{
    fileDataService->load(filePath);
...
void ConfigRepository::resetCache()
{
    cacheItems = {};
}
