...
ListService::get(bool keepHidden)
{
    ...
    if (!keepHidden) {
        lists.erase(std::remove_if(lists.begin(),
                                   lists.end(),
                                   [](const ListEntity& listEntity) { return *listEntity.isHidden(); }),
                    lists.end());
    }
...
ListEntity
ListService::find(const std::string& listName)
{
    return listRepository.find(listName);
}

void
ListService::add(const std::string& listName, const std::string& type, const std::string& sorting)
{
...
void
ListService::edit(const std::string& oldListName, std::string newListName)
{
...
void
ListService::remove(std::string& listName)
{
    listRepository.remove(listName);
}

bool
ListService::use(const std::string& listName)
{
...
void
ListService::subscribeToEvents(EventBus& eventBus)
{
...
std::string
ListService::getType(const std::string& listName)
{
...
std::vector<ListEntity>
ListService::sort(std::vector<ListEntity> listItems)
{
    std::sort(
        listItems.begin(), listItems.end(), [](ListEntity& a, ListEntity& b) { return *a.getName() < *b.getName(); });
