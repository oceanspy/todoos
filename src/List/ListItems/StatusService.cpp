...
StatusService::make()
{
    ...
    statuses = {
        ...
        StatusEntity::setFromVector({ std::to_string(TRIAGED),
                                      ...
                                      "false",
                                      "true" }),
        StatusEntity::setFromVector({ std::to_string(PAUSED),
                                      "3",
                                      "paused",
                                      "paused",
                                      "⏸️",
        ...
        StatusEntity::setFromVector(
            { std::to_string(CANCELLED), "7", "cancelled", "cancelled", "🚫", "GRAY", "", "true", "true", "true" })
    };
}

StatusEntity
StatusService::find(int statusId)
{
...
StatusEntity
StatusService::getStatusFromName(const std::string& name)
{
    for (const StatusEntity& statusEntity : statuses) {
    ...
    return StatusEntity::setFromVector(
        { std::to_string(-1), "-1", "invalid", "invalid", "❓", "2", "WHITE", "", "false", "false", "false" });
}

std::vector<std::string>
StatusService::getStatusesNames()
{
...
std::string
StatusService::getCommandNamesAsString()
{
...
bool
StatusService::isIdValid(int statusId)
{
...
bool
StatusService::isNameValid(const std::string& statusName)
{
...
int
StatusService::getIdFromName(const std::string& statusName)
{
    for (const StatusEntity& statusEntity : statuses) {
...
std::string
StatusService::getNameFromId(int statusId)
{
    for (const StatusEntity& statusEntity : statuses) {
        if (*statusEntity.getId() == statusId) {
...
bool
StatusService::isClosed(std::string statusName)
{
    for (const StatusEntity& statusEntity : statuses) {
